...
public class BlogFileService {
	...
	/*
	 * ServletContext
	 * tomcat 위에서 실행되는 Web Application의 모든 정보를 담고 있는 객체
	 * 이미 Spring Project에서는 ServletContext 클래스의 객체 인스턴스가 만들어져 있기 때문에 가져다 사용하기 위해서 Autowired로 연결(묶기)
	 */
	@Autowired
	...
	public List<BlogVO> selectAll() {
		...
		try {
			...
			//서버의 rootPath와 blogFile 이름을 묶어서 파일 관련 연산을 수행할 때 사용할 file 객체 생성
	...
	//blog에 글쓰기를 수행하고 저장하면 server rootPath에 blog.txt라는 파일로 저장
