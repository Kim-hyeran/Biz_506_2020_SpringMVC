...
public class NaverController {
	 ...
	 *  - 만약 client에서 보내는 변수 이름과 Controller에서 사용하는 변수 이름을 다르게 하고싶은 경우,
	 * 		@RequestParam(name="변수") String 이름 과 같이 작성
	 *  - client에서 해당 변수 값을 생략하고 전송했을 때, 오류를 최소화하기 위하여
	 *  	@RequestParam(name="이름", required=false, defaultValue="홍길동") String 이름 과 같이 작성
	 *  - client에서 이름 변수에 값을 보내지 않으면 기본값인 홍길동 문자열을 이름 변수에 할당한다.
	 */
	
	@RequestMapping(value="/search", method=RequestMethod.POST)
	public String search(@RequestParam(name="category", required=false, defaultValue = "BOOK") String category, @RequestParam(name="search_text") String search_text, Model model) {
