 ...
 * 
 * RestTemplate 기능을 사용하여 API 데이터를 가져오기 위한 핵심 기능을 사용하기 위한, 매우 중요한 Model 클래스
 * API를 통해 가져올 데이터(VO)들을 담을 List를 선언하는데, 이 때 변수의 이름은 API에서 보내주는 데이터의 이름을 참조하여 생성한다.
 */
@Alias("BookList")
...
public class Book_JSON_parent {
	...
	public String total;	// 검색 결과 문서의 총 개수를 의미한다.
	public String start;	// 검색 결과 문서 중, 문서의 시작점을 의미한다.
	public String display;	// 검색된 검색 결과의 개수이다.
