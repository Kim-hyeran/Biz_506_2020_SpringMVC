...
#### V3의 회원가입 문제점
 * 기본 CRUD는 모두 구현이 가능
 * 회원정보 중에서 민감한 정보(비밀번호)가 평문(Plain Text)으로 저장되어 누군가 회원정보 DB를 SELECT하게되면 ID와 Password가 노출되어 보안상 매우 위험한 상황

#### V3의 로그인 문제점
 * 로그인이 필요한 기능을 사용자가 (메뉴에서)선택하면, 로그인된 정보가 있는지 확인하여 없으면 로그인을 수행하도록 redirection이 된다.
 * 각각의 Controller/method에서 사용자 로그인 정보 검사를 수행
	- 만약 로그인이 필요한 기능이 많아진다면 불편함 발생  
	- 로그인을 검사하고 인증하는 절차에 해당하는 코드가 중복 작성되어야 한다.  

#### V4에서 회원가입, 로그인 개선사항
 * 회원정보 중 민감한 정보를 암호화하여 DB Table에 저장  
	- BCrypt 암호화  
 * 로그인 검사와 관련 코드 중 중복되는 코드를 **모아서 처리**  
	- intercepter 추가  
