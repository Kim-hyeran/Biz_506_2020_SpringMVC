...
public class ProductController {
	...
	//insert GET method : 상품정보 추가 anchor를 클릭했을 때 write 화면을 보여주는 method
	...
	/*
	 * @RequestParam("Query 이름")
	 *  - URL에 담긴 변수 이름과 실제 사용하는 파라미터 변수가 다를 때 @RequestParam()을 사용
	 *  - @RequestParam()은 spring 4.x 이상에서는 선택사항
	 *  - "Query 이름"과 파라미터 변수 이름이 같을 경우는 생략할 수 있다.
	 *  - 단, 이 Annotation은 primitive형이거나 wrapper(Integer, Character, String 등)형일 경우만 사용 가능
	 *  - 임의로 만든 VO를 파라미터로 사용할 때는 @ModelAttribute()를 사용해야 한다.
	 */
	 ...
	 * redirect
	 *  - request를 전환하는 개념
	 *  - server가 client에게 방금 요청을 수행하였으니 다시 한 번 /product/ URL로 request를 수행해달라는 요청
	 *  - 상태코드 304
	 */
	...
	/*
	 * update 실행 순서
	 * 1. update(GET) request를 수신, 이 때 매개변수는 PK값
	 * 2. findById(PK)를 호출하여 VO값 찾기
	 * 3. Model에 VO를 심기
	 * 4. write.jsp로 내려보내기
	 * 5. write.jsp는 Model에 담긴 VO를 input box에 보여주기
	 * 6. 사용자에게 변경할 데이터를 입력받기
	 * 7. 저장(Submit) 수행
	 * 8. update(POST) request를 수신, 이 때 매개변수는 VO
